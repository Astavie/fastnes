...
impl MOS6502 {
    fn cycle_start(&mut self) {
        if self.cycle == 0 || self.cycle == 1 {
            ...
            // instruction logic
            Instruction::from(self.opcode).cycle_start(self);
    ...
    fn cycle_end(&mut self) {
        ...
        // instruction
        if self.cycle == 0 {
            // instructions start from cycle 1 onwards
            // cycle 0 is specific for unknown opcode
        ...
        if self.cycle >= 2 {
            // instruction logic
            Instruction::from(self.opcode).cycle_end(self);
        } else {
            // start instruction next cycle
            self.cycle = 2;
        }
