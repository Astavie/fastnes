...
impl MOS6502 {
    ...
    pub fn dump_state(&self) {
        ...
        if cycle == 1 {
            ...
            if self.hardware_interrupt {
                ...
                println!(" ---- {} ---- ", Instruction::from(self.data));
                opcode = self.data;
            }
        }
    ...
    pub fn dump_rw(&self) {
        match self.rw {
            RW::Read(addr) => println!("    R ${:04x} = {:02x}", addr, self.data),
            RW::Write(addr) => println!("    W ${:04x} = {:02x}", addr, self.data),
        }
    }
}
...
impl MOS6502 {
    ...
    pub(crate) fn cycle_end(&mut self) {
        ...
        self.irq_sample = self.irq.get() > 0;
        self.nmi_sample = self.nmi_sample || self.nmi.get();
        self.nmi.set(false);
    }
}
...
pub(crate) fn nestest<const TEST: bool>(
    rom: [u8; 16384],
    instr: Rc<[Vec<fn(&mut MOS6502) -> bool>; 256]>,
) {
    // create chips and connect pins
    let mut mem = NESTest::from_rom(rom);
    ...
    if TEST {
        ...
        for _ in 0..7 {
            cpu.cycle_start();
            if let RW::Read(addr) = cpu.rw {
                cpu.data = mem.get(addr);
            }
            cpu.cycle_end();
            if let RW::Write(addr) = cpu.rw {
                mem.set(addr, cpu.data);
            }
        }
        ...
        loop {
            ...
            if cpu.cycle == None {
                ...
                assert_eq!(cpu.PC - 1, pc, "[line {}] PC:{:04X}", lineno, pc);
